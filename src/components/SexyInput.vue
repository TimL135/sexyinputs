<template>
    <!-- Button -->
    <button v-if="type == 'button'" :type="btnType" @click="affirm()" :class="btnClass">
        {{ btnText }}
    </button>
    <div class="input-contain mt-3 shadow-none" v-else>
        ...
        <input
            v-if="type != 'textarea' && type != 'select' && type != 'multiSelect' && type != 'button'"
            v-bind="$attrs"
            ...
            :style="[
                btnText || type == 'password' || sideInputType ? `border-radius: 0.5rem 0 0 0.5rem; width:${inputWidth}` : '',
