    ...
    <div class="simple-typeahead input-contain">
      ...
      >
        ...
        <div
          ...
          :class="listItemClass(noElementMessage)"
        >
          {{ noElementMessage }}
        </div>
...
const filteredItems = computed(() => {
  ...
  try {
    array = options.value?.filter((item) =>
      optionProjection.value?.(item).match(regexp)
    );
    if (!array.length)
      array = array.concat(
        options.value?.filter((item: any) => item.match(regexp))
      );
  } catch {
    array = [];
  }
...
function onInput(event: Event) {
  ...
  if (
    isListVisible.value &&
    currentSelectionIndex.value >= filteredItems.value.length
  ) {
  ...
  if (controlInput.value) {
    if (
      !filteredItems.value?.some(
        (e) => optionProjection.value(e) == modelValue.value
      )
    ) {
      updateValue("");
      return;
